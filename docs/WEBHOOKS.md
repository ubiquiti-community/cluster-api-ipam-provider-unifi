...
## Overview
Comprehensive validation and defaulting webhooks have been implemented for `IPPool` and `Instance` resources, following the Cluster API IPAM provider reference patterns.

## Files Created/Modified

### Webhook Implementations

#### 1. `internal/webhooks/ippool_webhook.go` (317 lines)
**Purpose:** Validation and defaulting webhook for IPPool resources
...
- **Create/Update Validation:**
  ...
  - Instance reference exists (Client.Get lookup)
...
**Kubebuilder Markers:**
```go
// +kubebuilder:webhook:path=/mutate-unifi-ipam-cluster-x-k8s-io-v1beta2-ippool
// +kubebuilder:webhook:path=/validate-unifi-ipam-cluster-x-k8s-io-v1beta2-ippool
```

#### 2. `internal/webhooks/instance_webhook.go` (237 lines)
**Purpose:** Validation and defaulting webhook for Instance resources
...
- **Delete Protection:**
  - Lists all IPPool resources in namespace
...
**Kubebuilder Markers:**
```go
// +kubebuilder:webhook:path=/mutate-unifi-ipam-cluster-x-k8s-io-v1beta2-instance
// +kubebuilder:webhook:path=/validate-unifi-ipam-cluster-x-k8s-io-v1beta2-instance
```
...
- **`manifests.yaml`** (generated by controller-gen):
  - `MutatingWebhookConfiguration` with 2 webhooks:
    - `default.instance.unifi.ipam.cluster.x-k8s.io` - CREATE, UPDATE operations
    - `default.ippool.unifi.ipam.cluster.x-k8s.io` - CREATE, UPDATE operations
  - `ValidatingWebhookConfiguration` with 2 webhooks:
    - `validation.instance.unifi.ipam.cluster.x-k8s.io` - CREATE, UPDATE, DELETE operations
    - `validation.ippool.unifi.ipam.cluster.x-k8s.io` - CREATE, UPDATE, DELETE operations
...
- **`webhook_in_instances.yaml`**:
  - Enables conversion webhook for Instance CRD
  - Path: `/convert`

- **`webhook_in_ippools.yaml`**:
  - Enables conversion webhook for IPPool CRD
  - Path: `/convert`

- **`cainjection_in_instances.yaml`**:
  - Adds cert-manager annotation for CA injection

- **`cainjection_in_ippools.yaml`**:
  - Adds cert-manager annotation for CA injection
...
5. **Reference Validation**: Instance must exist
...
4. **Delete Safety**: Blocks deletion if IPPools reference the instance
...
### Webhook Endpoints
- **Mutating**: `/mutate-unifi-ipam-cluster-x-k8s-io-v1beta2-{resource}`
- **Validating**: `/validate-unifi-ipam-cluster-x-k8s-io-v1beta2-{resource}`
...
```bash
kubectl annotate ippool my-pool ipam.cluster.x-k8s.io/skip-validate-delete-webhook=""
kubectl delete ippool my-pool
```
...
The webhook implementation provides comprehensive validation and defaulting for IPPool and Instance resources, preventing invalid configurations and protecting against accidental deletion of in-use resources. The implementation follows Cluster API patterns and Kubebuilder best practices for admission webhooks.
